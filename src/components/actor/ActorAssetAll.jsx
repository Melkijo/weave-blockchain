    ...
    useEffect(() => {
        ...
        const init = async () => {
            ...
            for (let i = 1; i <= totalAssets; i++) {
                ...
                const dataFabricDetails = await supplyChain.methods.fabricDetailsById(i).call();
                assetsWithPriceAndState.push({ id: i, price: infoAssets.price, state: infoAssets.state, time: tracker.time, address: tracker.actorAddress, buyStatus: tracker.buyStatus, rawMaterial: dataFabricDetails.rawMaterial, threadType: dataFabricDetails.threadType, fabricType: dataFabricDetails.fabricType, pattern: dataFabricDetails.pattern });
            }
    ...
    const buyAsset = async (assetId, price) => {
        ...
        try {
            ...
            if (result && assetId) {
                setSuccessMsg(
                    <span>
                        Asset with <strong>ID {assetId}</strong> succesfully purchased at price <strong>{convertWeiToEth(price)} ETH</strong>
                    </span>
                );
                setLoading(false);
    ...
    return (
        <div>
            ...
            <table className="table-auto w-full text-left">
                ...
                <tbody>
                    {
                        getDataloading ? "Getting Asset Data..." : (
                            filteredAssets.map((asset) => (
                                <tr key={asset.id} className="hover:bg-gray-100">
                                    ...
                                    <td className="border px-4 py-3">{convertWeiToEth(asset.price)} ETH
                                        ...
                                        <>
                                            {asset.state == 1 && (
                                                <td className="border px-4 py-3">{asset.rawMaterial}</td>
                                            )}
                                            {(asset.state == 2 || asset.state == 3) && (
                                                <td className="border px-4 py-3">{asset.threadType}</td>
                                            )}
                                            {(asset.state == 4 || asset.state == 5) && (
                                                <td className="border px-4 py-3">{fabricTypeName[asset.fabricType]} ({asset.pattern})</td>
                                            )}
                                        </>
                                    }
                                    ...
                                    <td className="border px-4 py-3">
                                        {asset.buyStatus == 0 && (
                                            <>
                                                {asset.state == 2 && role == 2 && asset.address != accounts[0] && (
                                                ...
                                                {asset.state == 3 && role == 3 && asset.address != accounts[0] && (
                                                    <button className="ml-2 bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition duration-200" onClick={() => buyAsset(asset.id, asset.price)}>{loading && id == asset.id ? "Loading..." : "Buy"}</button>
                                                )}
                                                {asset.state == 4 && role == 4 && asset.address != accounts[0] && (
                                                    <button className="ml-2 bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition duration-200" onClick={() => buyAsset(asset.id, asset.price)}>{loading && id == asset.id ? "Loading..." : "Buy"}</button>
                                                )}
                                                {asset.state == 5 && role == 5 && asset.address != accounts[0] && (
                                                    <button className="ml-2 bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition duration-200" onClick={() => buyAsset(asset.id, asset.price)}>{loading && id == asset.id ? "Loading..." : "Buy"}</button>
                                                )}
                                            </>
                                        )}
