...
const CreateThread = ({ isOpen, onClose, assetId }) => {
    ...
    const [threadQuality, setThreadQuality] = useState('');
    ...
    const handleChange3 = (e) => {
        const value = e.target.value;
        setThreadQuality(value);
    };
    const handleSubmit = async (e) => {
        e.preventDefault();
        setLoading(true);
        if (priceEth && threadType && threadQuality) {
            const weiPrice = web3.utils.toWei(priceEth.toFixed(18), "ether");
            try {
                const result = await supplyChain.methods.createThread(assetId, weiPrice, threadType, threadQuality).send({ from: accounts[0] });
                console.log(result);
                if (result && assetId) {
                    setSuccessMsg(
                        <span>
                            Asset with <strong>ID {assetId}</strong> succesfully processed at price {priceEth}
                        </span>
                    );
        ...
        if (!priceEth) {
            if (threadType == '') {
                setErrorMsg(`Thread Type must not be empty!`);
                setNotificationOpen(true);
            }
            else if (threadQuality == '') {
                setErrorMsg(`Thread Quality must not be empty!`);
                setNotificationOpen(true);
            }
            else if (price == '') {
                                ...
                                <div className="mb-1">
                                    ...
                                    <div className="mt-1 relative rounded-md shadow-sm">
                                        <input
                                            ...
                                            placeholder="ex. Cotton Combed 24s"
                                        />
                                    </div>
                                </div>
                                <div className="mb-1">
